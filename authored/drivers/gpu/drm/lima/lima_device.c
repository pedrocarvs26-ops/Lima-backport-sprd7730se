...
/*
 * 3.10 backport of lima_device.c:
 *  - <linux/device.h>/<linux/types.h> included before
 *    <linux/regulator/consumer.h> (3.10's consumer.h is not
 *    self-contained)
 *  - no reset framework (3.10 has no <linux/reset.h> use here; the SoC
 *    glue handles assert/deassert inside the power sequence if needed)
 *  - clocks are tolerant-optional: scx30g2 supplies them via DT
 *    ("bus" = clk_gpu_axi, "core" = clk_gpu); without DT clocks we warn
 *    and assume the platform left the GPU clocked
 *  - devm_regulator_get_optional() (3.13+) emulated with
 *    devm_regulator_get() + tolerant error handling
 *  - SoC power domain via lima_platform_power_on/off, executed BEFORE
 *    the clk init: on scx30g2 the GPU core clock control register
 *    (0x60100004) lives inside the GPU's own power domain, so it may
 *    only be touched after the domain is up (vendor mali400 driver
 *    order: sources/PLLs -> domain on -> AXI gate -> core select/div;
 *    the clk framework still enables PLL parents before the core clock)
 */
...
/* register offsets match the vendor node in sprd-scx35_sc7730.dtsi
 * (verified: gp +0x0000, l2 +0x1000, pmu +0x2000, gpmmu +0x3000,
 * ppmmu0/1 +0x4000/+0x5000, pp0/pp1 +0x8000/+0xa000) */
...
static int lima_clk_init(struct lima_device *dev)
{
	...
	if (IS_ERR(dev->clk_bus)) {
		dev_warn(dev->dev, "no bus clk (%ld), assuming always-on\n",
			 PTR_ERR(dev->clk_bus));
		dev->clk_bus = NULL;
	}
	...
	if (IS_ERR(dev->clk_gpu)) {
		dev_warn(dev->dev, "no core clk (%ld), assuming always-on\n",
			 PTR_ERR(dev->clk_gpu));
		dev->clk_gpu = NULL;
	}

	if (dev->clk_bus) {
		rate = clk_get_rate(dev->clk_bus);
	...
	if (dev->clk_gpu) {
		rate = clk_get_rate(dev->clk_gpu);
		dev_info(dev->dev, "mod rate = %lu\n", rate);
...
static int lima_regulator_init(struct lima_device *dev)
{
	...
	/* 3.10: devm_regulator_get_optional() came in 3.13 - emulate it.
	 * No "mali" supply is fine: the GPU rail is handled by the power
	 * domain sequence in lima_platform_sprd.c on this SoC. */
	dev->regulator = devm_regulator_get(dev->dev, "mali");
	if (IS_ERR(dev->regulator)) {
		ret = PTR_ERR(dev->regulator);
		dev->regulator = NULL;
		if (ret == -ENODEV || ret == -EPROBE_DEFER || ret == -ENOENT) {
			dev_info(dev->dev,
				 "no mali regulator (%d), continuing without\n", ret);
			return 0;
		}
...
int lima_device_init(struct lima_device *ldev)
{
	...
	/* power domain first: the GPU core-clock register lives inside the
	 * GPU's own power domain on scx30g2 (see file header) */
	err = lima_platform_power_on(ldev);
	if (err) {
		dev_err(ldev->dev, "platform power on fail %d\n", err);
		return err;
	}
	...
	if (!res) {
		dev_err(ldev->dev, "no register resource\n");
		err = -ENODEV;
		goto err_out4;
	}
...
void lima_device_fini(struct lima_device *ldev)
{
	...
	/* clocks off before the power domain, vendor shutdown order */
	lima_clk_fini(ldev);
