...
/*
 * 3.10 backport: drmP.h instead of drm_device.h (not split out in 3.10);
 * reset_control field dropped (no reset framework use in the backport);
 * clk/regulator/platform_device are forward-declared pointers only, so
 * this header stays cheap to include from every lima file.
 */
...
struct lima_device {
	...
	struct lima_platform_data *pdata;
